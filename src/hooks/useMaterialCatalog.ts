...
export function useCreateCatalogItem() {
  ...
  return useMutation({
    mutationFn: async (item: Omit<CatalogItem, 'id'>) => {
      ...
      // Check for duplicate by normalized description (case-insensitive)
      const { data: existing } = await supabase
        .from('material_catalog')
        .select('id, description')
        .ilike('description', description);
      if (existing && existing.some(e => normalizeText(e.description).toLowerCase() === description.toLowerCase())) {
        throw new Error('An item with this description already exists');
      }
